...
def test_get_items_with_data(test_client, mock_aws_services, auth_headers, sample_pantry_item):
    ...
    # First add an item
    add_response = test_client.post("/pantry/items", json=sample_pantry_item, headers=auth_headers)
...
def test_create_item_success(test_client, mock_aws_services, auth_headers, sample_pantry_item):
    ...
    response = test_client.post("/pantry/items", json=sample_pantry_item, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_create_item_invalid_data(test_client, auth_headers):
    ...
    response = test_client.post("/pantry/items", json=invalid_item, headers=auth_headers)
...
def test_update_item_success(test_client, mock_aws_services, auth_headers, sample_pantry_item):
    ...
    create_response = test_client.post(
        "/pantry/items", json=sample_pantry_item, headers=auth_headers
    )
    ...
    response = test_client.put(f"/pantry/items/{item_id}", json=updated_data, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_nonexistent_item(test_client, auth_headers, sample_pantry_item):
    ...
    response = test_client.put(
        "/pantry/items/nonexistent-id", json=sample_pantry_item, headers=auth_headers
    )
...
def test_delete_item_success(test_client, mock_aws_services, auth_headers, sample_pantry_item):
    ...
    # Delete the item
    response = test_client.delete(f"/pantry/items/{item_id}", headers=auth_headers)
    assert response.status_code == 200

    # Verify it's deleted
    get_response = test_client.get(f"/pantry/items/{item_id}", headers=auth_headers)
    assert get_response.status_code == 404


def test_delete_nonexistent_item(test_client, auth_headers):
    """Test deleting non-existent item"""
    response = test_client.delete("/pantry/items/nonexistent-id", headers=auth_headers)
    assert response.status_code == 404
...
def test_barcode_scan_endpoint(test_client, auth_headers, temp_image_file):
    ...
    with open(temp_image_file, "rb") as f:
        ...
        response = test_client.post("/pantry/scan-barcode", files=files, headers=auth_headers)
