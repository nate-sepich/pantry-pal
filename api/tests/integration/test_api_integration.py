...
class TestAPIIntegration:
    ...
    @pytest.mark.skipif(
        not os.getenv("INTEGRATION_AUTH_TOKEN"), reason="Integration auth token not available"
    )
    ...
    def test_api_error_handling(self, api_base_url):
        ...
        response = requests.post(
            f"{api_base_url}/pantry/items", data="invalid-json", headers=headers, timeout=30
        )
...
class TestAPIPerformance:
    ...
    def test_api_memory_usage_stability(self, api_base_url):
        ...
        ), f"Performance degradation detected: {early_avg}s -> {late_avg}s"
