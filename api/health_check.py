...
class HealthChecker:
    ...
    def check_endpoint(self, path: str, expected_status: int = 200, 
                      method: str = 'GET', headers: Dict[str, str] = None) -> bool:
    ...
    def run_health_checks(self) -> bool:
        ...
        for check in checks:
            ...
            print(f"{status} {check['method'].upper() if 'method' in check else 'GET'} {check['path']}")
    ...
    def check_performance(self, max_response_time: float = 5.0) -> bool:
        ...
        if slow_endpoints:
            ...
            avg_time = sum(r['response_time'] for r in self.results if r['response_time']) / len([r for r in self.results if r['response_time']])
...
def main():
    ...
    parser.add_argument('--max-response-time', type=float, default=5.0, help='Max acceptable response time')
    parser.add_argument('--output', help='Output JSON report to file')
    parser.add_argument('--fail-on-performance', action='store_true', help='Fail if performance check fails')
