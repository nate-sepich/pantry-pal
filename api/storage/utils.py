    ...
    try:
        with pantry_table.batch_writer() as batch:
            for item in items:
                ...
                batch.put_item(
                    Item={"PK": pk, "SK": f"PANTRY#{item.id}", **data, "expires_at": expires_at}
                )
    ...
    try:
        with pantry_table.batch_writer() as batch:
            for rec in items:
                ...
                batch.put_item(Item={"PK": pk, "SK": f"RECIPE#{rec.id}", **data})
...
def upsert_chat_meta(user_id: str, chat: ChatMeta) -> None:
    ...
    pk = f"USER#{user_id}"
    try:
        pantry_table.put_item(Item={"PK": pk, "SK": f"CHAT#{chat.id}", **chat.dict()})
    except ClientError as e:
